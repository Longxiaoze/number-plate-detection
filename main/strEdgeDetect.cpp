...
Rect strEdgeDetect(const Mat& inputImg) {
    ...
    for(auto& base_block : h_blocks) {
        for(int i = count; i < h_blocks.size(); i++) {
            if(base_block.peak < h_blocks[i].peak && h_blocks[i].top - base_block.bottom < 30) {
            ...
            else if(base_block.peak > h_blocks[i].peak && base_block.top - h_blocks[i].bottom < 30) {
                if(base_block.getWidth() > h_blocks[i].getWidth()) {
                    top = h_blocks[i].top;
